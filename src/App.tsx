                 ...
                 reports.map(r => (
                    <div key={r.id} className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col gap-3 shadow-sm">
                      ...
                      <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg text-sm border border-slate-100 dark:border-slate-800/80">
                        {r.reason}
                        {r.photo && <img src={r.photo} alt="Скриншот" className="mt-3 w-full rounded-lg object-contain bg-black/5" />}
                      </div>
...
function ReportModal({ targetId, onClose }: { targetId: string, onClose: () => void }) {
   ...
   const submit = () => {
      const targetUser = useStore.getState().players[targetId];
      if (targetUser && targetUser.isAdmin) {
         setError("Вы не можете кинуть жалобу этому игроку.");
         return;
      }
      if(!comment.trim()) {
           ...
           <div className="bg-white dark:bg-[#101423] border border-slate-200 dark:border-slate-800/80 rounded-2xl p-4 shadow-sm shrink-0">
               ...
               {isHost && (
                  <div className="w-full flex gap-2 mt-4">
                     <button className="flex-1 bg-rose-600 hover:bg-rose-500 active:scale-95 text-white font-bold py-3 px-4 rounded-xl shadow-md shadow-rose-900/20 text-sm transition-all flex items-center justify-center gap-2" onClick={() => socket?.emit('startGame')}><Play size={16} /> НАЧАТЬ ИГРУ</button>
                  </div>
               )}
           </div>
...
export default function App() {
  const { socket, myId, players, error, success, connect, friendRequests } = useStore();
  ...
  return (
    <>
      ...
      {friendRequests.map((req, i) => (
        <div key={req.fromId} style={{top: `${16 + i * 110}px`}} className="fixed left-4 bg-indigo-600/90 backdrop-blur text-white px-4 py-3 rounded-xl shadow-lg z-50 animate-in slide-in-from-left flex flex-col gap-2 min-w-[260px]">
           <div className="flex items-center gap-2">
             <Users size={16} className="text-indigo-200"/>
             <span className="font-bold text-sm">Заявка в друзья</span>
           </div>
           <p className="text-xs">
             <strong className="text-white">{req.fromName}</strong> хочет добавить вас в друзья.
           </p>
           <div className="flex gap-2.5 mt-1">
             <button onClick={() => { socket?.emit('acceptFriendRequest', req.fromId); useStore.getState().removeFriendRequest(req.fromId); useStore.getState().setSuccess("Заявка принята!") }} className="flex-1 bg-white hover:bg-slate-100 text-indigo-600 py-1.5 rounded-lg text-xs font-bold transition">Принять</button>
             <button onClick={() => { socket?.emit('rejectFriendRequest', req.fromId); useStore.getState().removeFriendRequest(req.fromId); }} className="flex-1 bg-indigo-800 hover:bg-indigo-900 text-indigo-100 py-1.5 rounded-lg text-xs font-bold transition">Отклонить</button>
           </div>
        </div>
      ))}
